                    ...
                    if (SpecialCharCheck.check(username) || SpecialCharCheck.check(password) || SpecialCharCheck.check(email)){
                        ...
                        try {
                            ...
                            switch (register_state) {
                                case 0 -> response.sendRedirect("ServerError");//这几个页面不归入ErrorPage1.html页面，防止为登录用户获得session
                                case 1 -> response.sendRedirect("EmailExist");
                                case 2 -> response.sendRedirect("SignUpSuccess");
                                case 3 -> response.sendRedirect("SignUpFail");
                            }
                        } catch (Exception e) {
